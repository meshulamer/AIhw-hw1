...
def run_astar_for_weights_in_range(heuristic_type: HeuristicFunctionType, problem: GraphProblem, n: int = 30,
    ...
    expanded = []
    for w in array:
        a_st = AStar(heuristic_type, w, max_nr_states_to_expand)
        res = a_st.solve_problem(problem)
        if res.stop_reason is StopReason.CompletedRunSuccessfully:
...
def mda_problem_with_astar_experiments():
    ...
    air_dist_a_star = AStar(MDAMaxAirDistHeuristic)
    res = air_dist_a_star.solve_problem(moderate_mda_problem_with_distance_cost)
    print(res)
